...
public class DirecaoDAO {
    ...
    private static final String SQL_INSERT =
            "INSERT INTO TB_REPLICACAO_DIRECAO(" +
                    ...
                    "SENHA_DESTINO, HABILITADO, PROCESSO_ID)" +
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

    private static final String SQL_UPDATE =
            "UPDATE TB_REPLICACAO_DIRECAO SET DIRECAO_ORIGEM = ?, DIRECAO_DESTINO = ?," +
                    "USUARIO_ORIGEM = ?, USUARIO_DESTINO = ?, SENHA_ORIGEM = ?, SENHA_DESTINO = ?," +
                    "HABILITADO = ?, PROCESSO_ID = ?" +
                    "WHERE ID = ?";

    private static final String SQL_DELETE =
            "DELETE TB_REPLICACAO_DIRECAO WHERE ID = ?";
    ...
    public void insert(TB_REPLICACAO_DIRECAO tb) throws SQLException {
        ...
        pstInsert.setString(7, tb.getSenha_origem());
        pstInsert.setString(7, tb.getSenha_destino());
        pstInsert.setBoolean(8, tb.isHabilitado());
        pstInsert.executeUpdate();
    }

    public void update(TB_REPLICACAO_DIRECAO processo) throws SQLException {
        pstUpdate.setLong(1, processo.getProcesso_id());
        pstUpdate.setString(2, processo.getDirecao_origem());
        pstUpdate.setString(3, processo.getDirecao_destino());
        pstUpdate.setString(4, processo.getUsuario_origem());
        pstUpdate.setString(5, processo.getUsuario_destino());
        pstUpdate.setString(7, processo.getSenha_origem());
        pstUpdate.setString(7, processo.getSenha_destino());
        pstUpdate.setBoolean(8, processo.isHabilitado());
        pstUpdate.setLong(9, processo.getId());
    ...
    private TB_REPLICACAO_DIRECAO map(ResultSet rs) throws SQLException {
        ...
        tb.setId(rs.getLong("PROCESSO_ID"));
        return tb;
    }
}
